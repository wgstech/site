...
export function LabeledInput({
  ...
  return (
    <fieldset className="group">
      <label className="text-lg font-medium text-slate-900" htmlFor={rest.name}>
        {label}
      </label>
      <div className="relative clear-both mt-1 flex flex-col items-center">
        {type === "input" ? (
          <>
            {Icon != null && (
              <Icon
                className="absolute top-1/2 left-2 size-6 -translate-y-1/2 text-slate-400 transition-colors group-focus-within:text-orange-400"
                weight={iconFill}
              />
            )}
